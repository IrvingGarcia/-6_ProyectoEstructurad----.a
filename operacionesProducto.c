...
extern int *aptrnumProductos;

aptrnumProductos = &numProductos

int opcionBuscar;
int *aptropBusc;

aptropBusc = &opcionBuscar
...
void mostrarListaProducto(Producto listaProducto[], int ordenar)
{
       ...
       for(int i = 0; i < *aptrnumProductos; i++)
       {
           arrayTemp[i] = i;
       }
             ...
             case _xNombre:
                  ...
                  for(int i = 0; i < *numProductos; i++)
                  {
                          if(strcmp(nombreBuscar, listaProducto[i].nombre) == 0)
                          {
                              ...
                              {
                                    printf("\nSizeof: %u\n", sizeof(* listaProducto));  /* Se coloca unsigned y no un  lu, ya que este es el valor que toma el argumento */
